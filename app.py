...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'the random string' #secret key is for session management and message flashing.

classes = ['System Detected Cancer as Negative',
            'System Detected Cancer as Positive']
...
def regback():
    ...
    #email = request.form["email"]
        ...
        print(email1)
        if email in email1:
            flash("email already exists","warning") #Uses flash to show success or error messages on the registration page.
            return render_template('ureg.html')
        if(pwd==cpwd):
...
def userlog():
    ...
    if request.method == "POST":
        ...
        print('p')
        mydb = mysql.connector.connect(host="localhost", user="root", password="", port=3307, database="cervical_cancer")
        cursor = mydb.cursor()
...
def upload():
    ...
    img=r"dataset"
    new_model = load_model(r"DenseNet_model.h5")
